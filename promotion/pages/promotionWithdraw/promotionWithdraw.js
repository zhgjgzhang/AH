...
Page({
  ...
  addWithdrawInfo: function (param, that){
    app.sendRequest({
  ...
  withdraw:function(){
    ...
    this.getPromotionInfo(function (res) {
      if (res.data.transfer_type.length == 2) {
        that.setData({
          hideSuccessDialog: false
        })
      } else if (res.data.transfer_type[0] == '0'){
        that.addWithdrawInfo({
          withdraw: that.data.withdrawCount,
          withdraw_type: 0
        }, that)
      } else {
        app.turnToPage('/promotion/pages/promotionWithdrawOffline/promotionWithdrawOffline?withdrawCount=' + that.data.withdrawCount);
      }
    })
  },
  ...
  withdrawToOther:function(){
    this.setData({
      hideSuccessDialog: true
    })
    app.turnToPage('/promotion/pages/promotionWithdrawOffline/promotionWithdrawOffline?withdrawCount=' + this.data.withdrawCount);
  },
})
